...
FSW_RESULT_ENUM tm_initialize(void)
{
	...
	tm_result = tm_monitor_task(TM_TASK_NAME_TM_SCHEDULER);
...
TM_RESULT_ENUM tm_start()
{
	...
	{
		os_task_spawn(gvTM_state.tasks[task_id].function,
			      gvTM_state.tasks[task_id].argument,
			      gvTM_state.tasks[task_id].priority,
			      gvTM_state.tasks[task_id].stackSize);
		if (os_result != OS_RESULT_OKAY)
		{
...
bool tm_running(TM_TaskId task_id)
{
	...
	{
		OS_sem_take(gvTM_state.tasks[task_id].semaphore, WAIT_FOREVER);
	}
...
void tm_scheduler_task(void)
{
	while (gvTM_state.continue_running)
	{
		OS_RESULT_ENUM os_result =
			os_sem_take(gvTM_state.schedule_semaphore, WAIT_FOREVER);
		...
		if (os_result == OS_RESULT_OKAY)
		{
			for (int task_id = 0; task_id < TM_MAX_TASKS; task_id++)
			{
				...
				OS_TASK_STATUS_ENUM task_status = 
					os_task_status(gvTM_state.tasks[task_id]);
...
void tm_process_task(TM_TASKSTATUS_ENUM status, TM_TaskId task_id)
{
	switch (status)
	{
		...
		case TM_TASKSTATUS_SCHEDULE:
			...
			{
				OS_STATUS os_status =
					os_sem_give(gvTM_state.tasks[task_id].schedule_sem, FSW_NO_WAIT);
				if (os_status == OS_STATUS_OKAY)
				{
			    ...
			    int priority)
{
	...
	if (tm_result == TM_RESULT_OKAY)
	{
		gvTM_state.type = TM_TASKTYPE_PERIODIC;
		gvTM_state.ticks = 0;
		gvTM_state.function = task_function;
		gvTM_state.argument = task_argument;
		gvTM_state.schedule_period = period;
		gvTM_state.heartbeat_period = heartbeat_period;
		gvTM_state.stack_size = stack_size;
		gvTM_state.priority = priority;
		             ...
		             int priority);
{
	...
	if ((task_name == NULL) || (task_function == NUL))
	{
		tm_result = TM_RESULT_NULL_POINTER;
	}
	...
	if (tm_result == TM_RESULT_OKAY)
	{
		gvTM_state.type = TM_TASKTYPE_EVENT;
		gvTM_state.ticks = 0;
		gvTM_state.function = task_function;
		gvTM_state.argument = task_argument;
		gvTM_state.schedule_period = 0;
		gvTM_state.heartbeat_period = heartbeat_period;
		gvTM_state.stack_size = stack_size;
		gvTM_state.priority = priority;
				...
				int period)
{
	...
	if (tm_result == TM_RESULT_OKAY)
	{
		gvTM_state.type = TM_TASKTYPE_CALLBACK;
		gvTM_state.ticks = 0;
		gvTM_state.function = task_function;
		gvTM_state.argument = task_argument;
		gvTM_state.schedule_period = 0;
		gvTM_state.heartbeat_period = 0;
		gvTM_state.stack_size = 0;
		gvTM_state.priority = 0;
...
TM_RESULT_ENUM tm_monitor_task(char *task_name, int task_id)
{
	...
	if (tm_result == TM_RESULT_OKAY)
	{
		gvTM_state.type = TM_TASKTYPE_MONITOR;
		gvTM_state.ticks = 0;
		gvTM_state.function = NULL;
		gvTM_state.argument = 0;
		gvTM_state.schedule_period = 0;
		gvTM_state.heartbeat_period = 0;
		gvTM_state.stack_size = 0;
		gvTM_state.priority = 0;
